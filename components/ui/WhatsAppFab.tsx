"use client";

import { useScrolledPast, useFabAvoidance } from "@/lib/hooks/useSentinel";

/**
 * FLOATING WHATSAPP BUTTON
 * ========================
 * Appears once the hero is behind you, and gets out of the way rather than
 * sitting on top of anything.
 *
 * ── STAYING OUT OF THE WAY ──────────────────────────────────────────────────
 * Any element marked `data-fab-avoid` — the ticket form, the apply section,
 * any future mobile sticky CTA — pushes the button off screen while it is in
 * the lower part of the viewport. So it can never cover a form field or
 * compete with the primary action.
 *
 * It also sits above `env(safe-area-inset-bottom)`, so it clears the iOS home
 * indicator and Android gesture bar instead of hiding behind them.
 *
 * When hidden it is `aria-hidden` with `pointer-events: none` and removed from
 * the tab order, so keyboard and screen-reader users are not offered a control
 * they cannot see.
 */

/**
 * Props rather than a config import: this is a client component, and
 * importing the config module here pulled all 40 KB of site copy into the
 * browser bundle to supply one URL.
 */
export function WhatsAppFab({ href, label }: { href: string; label: string }) {
  const past = useScrolledPast("top");
  const avoiding = useFabAvoidance();

  const visible = past && !avoiding;

  return (
    <a
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      aria-label={label}
      aria-hidden={!visible}
      tabIndex={visible ? 0 : -1}
      className={[
        "fixed right-4 z-40 grid size-14 place-items-center rounded-full",
        "bg-cta text-cta-ink border-cta-ink/15 border-2",
        "transition-[transform,opacity] duration-300 ease-out",
        "motion-reduce:transition-none",
        "hover:scale-105 active:scale-95 motion-reduce:hover:scale-100",
        visible
          ? "translate-y-0 opacity-100"
          : "pointer-events-none translate-y-24 opacity-0",
      ].join(" ")}
      style={{
        // Clears the iOS home indicator and the Android gesture bar.
        bottom: "calc(1rem + env(safe-area-inset-bottom, 0px))",
      }}
    >
      <svg
        width="26"
        height="26"
        viewBox="0 0 24 24"
        fill="currentColor"
        aria-hidden="true"
        focusable="false"
      >
        <path d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91c0 1.75.46 3.45 1.32 4.95L2 22l5.25-1.38a9.9 9.9 0 0 0 4.79 1.22h.01c5.46 0 9.91-4.45 9.91-9.91 0-2.65-1.03-5.14-2.9-7.01A9.82 9.82 0 0 0 12.04 2Zm0 18.15h-.01a8.2 8.2 0 0 1-4.19-1.15l-.3-.18-3.12.82.83-3.04-.2-.31a8.22 8.22 0 0 1-1.26-4.38c0-4.54 3.7-8.23 8.25-8.23a8.2 8.2 0 0 1 8.24 8.24c0 4.54-3.7 8.23-8.24 8.23Zm4.52-6.16c-.25-.12-1.47-.72-1.69-.81-.23-.08-.39-.12-.56.13-.16.24-.64.8-.78.97-.14.16-.29.18-.54.06-.25-.13-1.05-.39-1.99-1.23-.74-.66-1.23-1.47-1.38-1.72-.14-.25-.01-.38.11-.5.11-.11.25-.29.37-.43.13-.15.17-.25.25-.41.08-.17.04-.31-.02-.43-.06-.12-.56-1.34-.76-1.84-.2-.48-.4-.42-.56-.43h-.48c-.16 0-.43.06-.65.31-.22.25-.85.84-.85 2.03 0 1.2.87 2.35.99 2.51.12.17 1.72 2.62 4.16 3.68.58.25 1.03.4 1.39.51.58.19 1.11.16 1.53.1.47-.07 1.47-.6 1.67-1.18.21-.58.21-1.07.15-1.18-.06-.11-.22-.17-.47-.29Z" />
      </svg>
    </a>
  );
}

export default WhatsAppFab;
